...
def fig_choropleth(df):
    """World choropleth — orthographic globe, white theme, drag-to-rotate only."""
    ...
    fig = px.choropleth(
        ...
        color_continuous_scale=[
            [0, "#F5F5F5"], [0.15, "#FFE0CC"], [0.3, "#FFCCBC"],
            [0.5, "#FF8A65"], [0.7, "#FF7043"], [1, "#FF4500"],
        ],
        hover_name="birth_country",
    )
    _base_layout(fig, "Nobel Prizes by Birth Country", height=600)
    fig.update_geos(
        projection_type="orthographic",
        projection_rotation=dict(lon=-30, lat=25, roll=0),
        bgcolor=BG_COLOR,
        lakecolor="#F8FAFC",
        landcolor="#FFFFFF",
        showocean=True, oceancolor="#F8FAFC",
        showframe=False,
        coastlinecolor="#CBD5E1", coastlinewidth=1,
        countrycolor="#E2E8F0", countrywidth=0.6,
        showlakes=True,
        showrivers=False,
        showcountries=True,
    )
...
def fig_globe_by_category(df, category=None):
    """3D globe filtered by a specific category, showing all if None."""
    if category:
    ...
    counts = sub.groupby(["birth_country", "category"]).size().reset_index(name="n")
    country_totals = sub.groupby("birth_country").size().reset_index(name="total")

    fig = go.Figure()

    cats = sorted(sub["category"].unique())
    cat_colors = {c: COLORS.get(c, "#888") for c in cats}
    ...
    for cat in cats:
        ...
        for _, row in cat_data.iterrows():
            ...
            lats.append(coords[0])
            lons.append(coords[1])
            sizes.append(max(6, row["n"] * 3.5))
            hover.append(f"<b>{row['birth_country']}</b><br>{cat}: {row['n']}")
    ...
    _globe_layout(fig, f"Category Globe{title_suffix}", 680)
    return fig


def fig_globe_timeline(df):
    """
    3D globe showing laureates placed on the globe over decades,
    with a slider to filter by decade.
    """
    decades = sorted(df["decade"].unique())
    valid = df.dropna(subset=["birth_country"]).copy()
    valid = valid[valid["birth_country"].apply(lambda c: _get_coords(c) is not None)]

    fig = go.Figure()

    # Add all decades as traces
    for dec in decades:
        dec_data = valid[valid["decade"] == dec]
        lats, lons, hover, colors = [], [], [], []
        for _, row in dec_data.iterrows():
            lat, lon = _get_coords(row["birth_country"])
            lats.append(lat)
            lons.append(lon)
            hover.append(
                f"<b>{row['name']}</b><br>"
                f"{row['category']} ({int(row['award_year'])})<br>"
                f"{row['birth_country']}"
            )
            colors.append(COLORS.get(row["category"], "#888"))
        ...
        fig.add_trace(go.Scattergeo(
            ...
            marker=dict(size=8, color=colors, opacity=0.8,
                        line=dict(width=0.3, color="#FFFFFF")),
            hovertext=hover, hoverinfo="text",
            name=f"{dec}s", visible=False,
        ))

    # Make last decade visible by default
    if len(decades) > 0:
    ...
    for i, dec in enumerate(decades):
        step = dict(
            label=f"{dec}s",
            method="update",
            args=[{"visible": [j == i for j in range(len(decades))]}],
        )
        steps.append(step)

    fig.update_layout(
        sliders=[dict(
            active=len(decades) - 1,
            currentvalue=dict(prefix="Decade: ", font=dict(family="JetBrains Mono, monospace", size=14)),
            pad=dict(t=40),
            steps=steps,
    ...
    _globe_layout(fig, "Nobel Laureates by Decade — Interactive Globe", 720)
    return fig


def fig_globe_connections(df):
    """3D globe with arcs connecting countries that share categories."""
    # Get top 10 countries
    top = df.groupby("birth_country").size().nlargest(10).index.tolist()
    sub = df[df["birth_country"].isin(top)].copy()
    ...
    for country in top:
        ...
        if coords is None:
            continue
        n = int(sub[sub["birth_country"] == country].shape[0])
        fig.add_trace(go.Scattergeo(
            lon=[coords[1]], lat=[coords[0]],
            mode="markers+text",
            marker=dict(size=max(8, n * 0.15), color="#FF4500", opacity=0.9,
                        line=dict(width=1, color="#000000")),
            text=[f"{country} ({n})"],
            textposition="top center",
            textfont=dict(size=9, family="JetBrains Mono, monospace", color="#000000"),
            showlegend=False, hoverinfo="text",
            hovertext=f"<b>{country}</b><br>{n} prizes",
        ))

    # Connection arcs using ploty lines in geoprojection
    pairs_done = set()
    for cat in df["category"].unique():
        cat_countries = sub[sub["category"] == cat]["birth_country"].unique()
        for i, c1 in enumerate(cat_countries):
            for c2 in cat_countries[i + 1:]:
                pair = tuple(sorted([c1, c2]))
                if pair in pairs_done:
                    continue
                pairs_done.add(pair)
                coords1 = _get_coords(c1)
                coords2 = _get_coords(c2)
                if coords1 is None or coords2 is None:
                    continue

                fig.add_trace(go.Scattergeo(
                    lon=[coords1[1], coords2[1]], lat=[coords1[0], coords2[0]],
                    mode="lines",
                    line=dict(color="rgba(255, 69, 0, 0.4)", width=1.5),
                    showlegend=False, hoverinfo="skip",
                ))

    _globe_layout(fig, "Country Connections — Shared Prize Categories", 700)
    return fig
